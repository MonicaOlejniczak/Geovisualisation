...
\label{sec:system}
	...
	The implementation of these visualisations has met all but one of the advanced system deliverables defined in Section~\ref{sec:project_definition}. It can also be observed that this project fulfils many goals by:

	\begin{itemize}
		\item Implementing the visualisations with generated, real and live datasets which are capable of being displayed in a time efficient manner.
		\item Integrating the system with the final year project as a part of the teaching analytics component.
		\item Conducting a user study and performance analysis outlined in Section~\ref{sec:system_evaluation} to identify user issues and performance concerns.
	\end{itemize}
...
\label{sec:system_evaluation}
	...
	\label{sec:user_study}
		...
		The process began by asking the participant to complete a consent form. A brief overview of each visualisation and the dataset they represent was provided after the user completed this form. The participant was given task instructions and encouraged to ask questions throughout the evaluation. They were then able to experiment with the visualisations in any order and asked to complete a questionnaire. On average the participant took approximately seven minutes to complete the study.
