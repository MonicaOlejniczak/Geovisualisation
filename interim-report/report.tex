...
\title{Exploring 3D Geovisualisation Techniques and their Applications for Large Datasets using HTML5 and WebGL}
...
\setlist[description, 2]{wide=15pt}
\setlist[description, 3]{wide=30pt}
...
\begin{document}
	...
	\label{sec:introduction}
		...
		\todo{change to geovisualisation}

		Data visualisation is the presentation of information in a visual context. It is particularly important in data analytics, since humans are able to make predictions and recognise patterns, trends and correlations more easily \citep{grinstein2002introduction}. Recognising these relationships is critical when working with large datasets, as the mass of information needs to be filtered and interpreted, so analysts can make informed decisions \citep{hendley1995case}. This task can be resolved by creating effective data visualisations, as they have the potential to lead to analytic discovery or be used for data reduction \citep{rohrer1997web}. 
	...
	\label{sec:background}
		...
		\subsection{HTML5 and WebGL} {

			HTML5 is a standardised markup language for describing web documents \citep{w3c2014html5}, providing a common platform for applications to deliver multimedia content to the web \citep{lawson2011introducing, chaturvedi2015web}. WebGL is a cross-platform web standard for a 3D graphics API derived from OpenGL, which has been exposed through the HTML5 canvas element \citep{marrin2011webgl} as a drawing context. It is widely supported in modern browsers, without needing additional plug-ins or extensions, and is designed for building dynamic applications that require 3D visualisations \citep{chaturvedi2015web, marrin2011webgl, parisi2012webgl}. Rendering such visualisations in real-time is possible due to the accelerated graphics rendering provided by WebGL, which utilises the graphics card memory on a device \citep{chaturvedi2015web} to perform multiple operations in parallel to one another. In this work, HTML5 and WebGL will be utilised to visualise geographical data, which ensures that this project is cross-platform and capable of rendering complex visualisations through accelerated graphics rendering.

		}

		\subsection{Geovisualisation} {
	...
	\label{sec:project_schedule}
		...
		The project plan, which was outlined in Section~\ref{sec:project_plan}, is to be completed according to the project schedule. Unfortunately, it is quite probable that no work will be undertaken towards the individual project during the mid-year recess. This is due to the need to prepare for and attend RoboCup 2015, China. It has therefore been omitted from the schedule, which has been demonstrated through the Gantt chart in Appendix~\ref{app:gantt_chart}. This chart clearly highlights each milestone that is critical to the success of this course:
		
		\begin{description}
			\item[Meetings:] The meetings required in this project.
			\begin{description}[itemsep=-1ex, topsep=0pt, partopsep=0pt]
				\item[Group:] These weekly meetings are held during the assigned timetable slot for the course and will involve discussing assessment items, progress and assigning tasks to group members.
				\item[Supervisor:] Progress update meetings have been scheduled with the project supervisor on a fortnightly basis. These meetings are designed acquire advice and to ensure that the individual project is going to plan.
				\item[Client:] Client meetings will be scheduled when necessary but should be approximately every two or three weeks and will aim to discuss progress, content and ideas.
			\end{description}
			\item[Group participation:] The set of assessment items that comprise group work.
			\begin{description}[itemsep=-1ex, topsep=0pt, partopsep=0pt]
				\item[Work:] The continous set of work completed for the group project, such as implementation or testing.
				\item[Progress reports:] These reports are due at the end of every week and should indicate what has been achieved during that week.
				\item[Group system demonstration:] The system demonstration should be prepared for a week in advance. This will include discussing the requirements of the demonstration, what has currently been achieved by all members and assigning tasks during the previous group meetings. Any remaining functionality that is required for the demonstration will be implemented during this week.
